...
class ResumeUploadView(APIView):
    ...
    def extract_name(self, text):
        """
        Extract name from resume text with improved robustness.
        Tries multiple strategies to find the person's name.
        """
        ...
        for line in lines[:20]:  # Check first 20 lines
            ...
            if match:
                ...
                # Remove trailing junk like "- Resume", "- CV", etc.
                name = re.sub(r'\s*[-–—]\s*(Resume|CV|Profile|Summary|r\.pdf).*', '', name, flags=re.IGNORECASE).strip()
                if name and 5 <= len(name) <= 50:  # Reasonable name length
                    return name
        
        # Strategy 2: Look in first 10 non-empty lines for candidate names
        # Filter out common header words and look for proper names
        non_empty_lines = [line.strip() for line in lines if line.strip() and len(line.strip()) > 4]
        
        # Words that typically appear in headers, not names
        exclude_keywords = [
            'email', 'phone', 'linkedin', 'github', 'website', 'portfolio',
            'address', 'summary', 'objective', 'skills', 'experience', 'education',
            'projects', 'certifications', 'achievements', 'languages', 'professional',
            'resume', 'cv', '@', 'http', 'www', '---', 'date', 'year'
        ]
        
        for i in range(min(10, len(non_empty_lines))):
            line = non_empty_lines[i]
            
            # Remove trailing junk (like "- Resume", "- CV", etc.)
            line_cleaned = re.sub(r'\s*[-–—]\s*(Resume|CV|Profile|Summary|r\.pdf).*', '', line, flags=re.IGNORECASE).strip()
            ...
            # - Between 5 and 50 characters (reasonable name length)
            # - Contains mostly letters and spaces (allow hyphens, apostrophes)
            # - Can be Title Case OR All Caps (but not all lowercase)
            if (5 <= len(line_cleaned) <= 50 and
                re.match(r'^[A-Za-z\s\.\'\-]+$', line_cleaned) and
                not line_cleaned.islower() and
                len(line_cleaned.split()) >= 1):  # At least one word
                return line_cleaned
        
        # Strategy 3: Fallback - return "Name not found"
